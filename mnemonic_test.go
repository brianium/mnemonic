...
func TestEnglishMnemonics(t *testing.T) {
	englishVectors := vectors["english"]
	for _, vector := range englishVectors {
		inputEntropy := vector[0]
		sentence := vector[1]
		seed := vector[2]
		ent, err := entropy.FromHex(inputEntropy)
		if err != nil {
		...
		mnemSeed := mnem.GenerateSeed("TREZOR")
		actualSeed := mnemSeed.ToHex()
		if actualSeed != seed {
			t.Errorf("\nExpected seed: %s\nActual seed: %s\n", seed, actualSeed)
		}
	}
...
func BenchmarkRandomEnglishMnemonicCreation(b *testing.B) {
	for i := 0; i < b.N; i++ {
		NewRandom(256, English)
	}
}
...
// test vectors taken from https://raw.githubusercontent.com/trezor/python-mnemonic/master/vectors.json
